// Get the objects we need to modify
let updateLocationForm = document.getElementById('/update-location-ajax/');

// Modify the objects we need
updateLocationForm.addEventListener("submit", function (e) {
   
    // Prevent the form from submitting
    e.preventDefault();

    // Get form fields we need to get data from
    let inputlocationID = document.getElementById("mySelect");
    let inputdescription = document.getElementById("update-input-description");
    // Get the values from the form fields
    let locationIDValue = inputlocationID.value;
    let descriptionValue = inputdescription.value;
    console.log("inputlocationIDValue",locationIDValue, "inputdescriptionValue", descriptionValue);
    // currently the database table for Locations does not allow updating values to NULL
    // so we must abort if being bassed NULL for locationDescription


    // Put our data we want to send in a javascript object
    let data = {
        locationID: locationIDValue,
        description: descriptionValue,
    }
    console.log(data,"data");
    // Setup our AJAX request
    var xhttp = new XMLHttpRequest();
    xhttp.open("PUT", "/update-location-ajax/", true);
    xhttp.setRequestHeader("Content-type", "application/json");

    // Tell our AJAX request how to resolve
    xhttp.onreadystatechange = () => {
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            // Add the new data to the table
            updateRow(xhttp.response, locationIDValue, descriptionValue);
        }
        else if (xhttp.readyState == 4 && xhttp.status != 200) {
            console.log("There was an error with the input.")
        }
    }

    // Send the request and wait for the response
    xhttp.send(JSON.stringify(data));
})

function updateRow(data, locationID, description){
    let parsedData = JSON.parse(data);
    console.log("parsed Data", parsedData);
    let table = document.getElementById("location-table");

    for (let i = 0, row; row = table.rows[i]; i++) {
       //iterate through rows
       //rows would be accessed using the "row" variable assigned in the for loop
       if (table.rows[i].getAttribute("data-value") == locationID) {

            // Get the location of the row where we found the matching person ID
            let updateRowIndex = table.getElementsByTagName("tr")[i];

            // Get td of description value
            let td = updateRowIndex.getElementsByTagName("td")[2];

            // Reassign locationdescription to our value we updated to
            td.innerHTML = description; 
       }
    }
}
